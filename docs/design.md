...
已实现的静态硬件扩展：SoC version、Chip version 和带依据的理论算力与实时指标分离，沿用硬件适配器的只读查询入口，存入节点元数据。纳管、重新检测时更新；不做时间聚合，不参与自动回收。字段口径、实际查询命令、算力登记和失效规则见 [硬件规格说明](hardware-profile.md)。
