...
2026-09-12。设计 v6 已进入实现；本文件描述当前代码，设计文档仍保留目标规格。已通过现有 SSH 接入 5 台真实节点，未安装常驻组件；已运行不占 NPU 算力的临时自检任务，现场结果见 [实机验收记录](real-node-acceptance.md)。

界面更新：任务中心暂时隐藏，保留执行模块与接口；当前入口为登录、实时信息、集群管理、机器申请。当前 UI 保留参考 [codex-reset.com](https://codex-reset.com/) 的细网格、矩形边框、括号按钮和终端式登录面板，配色按用户反馈恢复为原有深绿侧栏、草绿点缀与浅色统计卡，纳管节点卡片使用深色数字。浅底文字使用深绿或深琥珀；中文微软雅黑、英文/数字 Consolas，保留桌面与移动端布局及减少动效偏好。无新增依赖或第三方站点脚本。

## 已实现的链路

硬件规格新增系统产品、板卡产品、逐设备 SoC version / Chip version，以及带来源的 A3 FP16 双芯模块算力登记。五台实机的 SoC 已全部核验；560T / 752T 对应关系未确认，不预填。字段口径、查询命令和依赖见 [硬件规格说明](hardware-profile.md)。
...
| 纳管检测 | A2/A3 纳管代表卡双向抽检，申请时选中卡全组合复验与 TLS 检查；共享挂载别名发现与跨节点写读校验 |
...
浏览器界面仅显示纳管实机。原预览测试和本次验收的临时申请/任务已删除；回归测试中的示例节点仅通过拦截 GET 响应或独立测试库验证，不写入平台库。
...
最终全套运行 117 项测试：中心机 113 项通过、4 项 Linux 协议测试在 Windows 跳过；这 4 项另在真实 Linux 节点执行并全部通过。MySQL 集成使用独立临时库；前端生产构建与真实 API 桌面/移动端浏览器验收通过。

前端已做 TypeScript 检查、生产构建和真实 API 浏览器流程，覆盖桌面/移动端、空集群、申请/取消、任务/日志/取消、刷新和退出。硬件解析同时覆盖合成边界样本与本次 5 台实机的脱敏 npu-smi 格式；A2/A5 没有因此视为实机验收。

Linux 本地任务协议测试在 tests/test_task_scripts.py，验证重复启动、关闭标记、后台进程超时与日志上限；Windows 环境明确跳过；本次另外上传到 159 节点的独立临时目录实测，4 项均通过。修复了两万多个进程时 Bash 全机扫描过慢的问题，改用节点已有 procps（ps）按任务 session 筛选再核验 /proc 身份，未新增常驻组件。临时脚本、测试进程及文件已清理。

本次已验证实际设备/PID/容器、A3 映射及共享盘；新增其他机型或部署时仍需核验：
