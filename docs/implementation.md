...
界面更新：任务中心暂时隐藏，保留执行模块与接口；当前入口为登录、实时信息、集群管理、机器申请。当前 UI 参考 [codex-reset.com](https://codex-reset.com/) 的终端风格：保留浅色主区域，采用深绿侧栏、荧光绿点缀、细网格、矩形边框、括号按钮和终端式登录面板。浅底文字使用深绿或深琥珀；中文微软雅黑、英文/数字 Consolas，保留桌面与移动端布局及减少动效偏好。无新增依赖或第三方站点脚本。
