...
def build_services(settings):
    ...
    return SimpleNamespace(db=db,settings=settings,identity=Identity(db,settings),inventory=inventory,
        transport=transport,telemetry_transport=telemetry_transport,adapters=adapters,catalog=catalog,telemetry=telemetry,resources=resources,reporting=Reporting(db,settings),
