...
当前页面：用户名登录、集群管理、所有节点实时信息、机器申请。支持整机/部分卡申请、30 分钟交付保护、按实际 AI Core 活动保留资源、连续空闲后的清理释放、真实 PID/Docker 容器、互联及共享盘检测、SoC 采集和带依据的算力规格登记。“机器申请”中可选创建 PR 容器任务，配置独立环境、并行或依赖作业、日志与产物。nightly/weekly 预置逐项验收接入，当前 Qwen3 样例的真实状态见[样例记录](docs/nightly-qwen3.md)。
...
每个任务包含多个 job，依赖通过作业图连线设置。页面按前置校验、执行、后续检查展示，各自关联具名 Shell/Python/YAML 附件，点击文件旁的“编辑”查看并修改全文；保存会同步当前任务内所有同名附件。启动命令显示如何调用文件，模型、数据集、压测参数在预置的 YAML 中修改。Qwen3 预置使用独立的创建容器、服务端安装和客户端安装脚本，以及每个执行阶段独立的作业脚本。环境准备安装所选 vLLM-Ascend 源码及其 .github 中指定的匹配 vLLM；不解析 nightly 用例。每个步骤仅展示自身需要的附件，不再汇总所有文件；服务端不附带 AISBench 工具。`nightly_environment.py` 是 Hive 的历史安装封装，新预置不再使用。每个 job 默认超时 10 分钟，可修改。提交时冻结实际编辑内容、SHA-256 与原文件摘要；不会修改公共归档或上游代码。上传上游文件可使用相对路径消除重名，YAML 由附件中的业务 runner 处理，Hive 不解释业务配置。
...
任务固定新建环境，页面不再提供“复用已有环境”“保留环境”及保留天数选项。所有作业结束并核验清理完成后关闭容器、归还资源；如果停止状态或连接不明确，会保留占用继续核验。历史任务和归档记录保留，新任务不会接管已有空间。
...
镜像输入支持本机名称、资源映射别名，以及从 [Quay vllm-ascend 标签](https://quay.io/repository/ascend/vllm-ascend?tab=tags) 分页选择；中心机需要访问 quay.io 的公开标签 API。优先使用节点已有镜像；仅明确选择的 `quay.io/ascend/vllm-ascend:<tag>` 本地缺失时尝试拉取一次，设置 10 分钟时限（另有收尾缓冲），失败显示原因，不无限重拉。节点需能访问 Quay。最终冻结镜像 ID 后启动容器。拉取期间取消需等待已发出操作确认或超时。预置的具名 bootstrap 脚本也可编辑；新预置附带完整 Docker 创建脚本，不调用节点上的业务脚本。镜像由 Hive 准备并冻结后，脚本创建容器，设备与挂载参数可在附件中编辑。

### 12.2 外部脚本参数与结果

启动命令、Shell 和 Python 使用六类 Hive 环境变量：`HIVE_SOURCE_DIR`、`HIVE_NODE0_IP` 等节点 IP、`HIVE_CONTAINER0_NAME` 等容器名，以及作业阶段的 `HIVE_TASK_ID`、`HIVE_JOB_ID`、`HIVE_OUTPUT_DIR`。页面给出环境/节点/容器编号对应表；端口和业务参数在公共脚本或 YAML 中配置。完整语义见 [工作流变量](docs/workflow-variables.md)。

权重、数据集和软件包路径通过 `hive_resource` 公共 Shell 函数查询，再以参数传给业务 Python 文件；不再注入额外的 Hive JSON 环境变量。依赖安装与核验均由可查看、可编辑的任务文件完成。

每项产物默认选择该 job 环境的所有节点，也可在下拉框按“服务器 / 容器”多选其他已配置目标；增加节点时默认目标同步扩展。配置名称和容器内绝对路径，一行一项，推荐使用 `$HIVE_OUTPUT_DIR/result.json`；根目录为 `/var/tmp/hive/outputs/<任务ID>/<作业实例ID>`。普通文件原样保存，目录自动打包为 `tar.gz` 下载；不同节点/容器分别归档，同一目标不会因多个 job 实例重复收集。Qwen3 预置同时保留 AISBench 完整 outputs 结果树及摘要 JSON/CSV。包含 metrics 字段的 JSON 按下述通用协议展示，Hive 不计算业务指标或阈值：
...
每个用例目录包含 `source.json`（上游完整 commit、nightly YAML 路径、预置人员及脚本清单）、`workflow.json`（资源、环境及 job 定义）、独立 Shell/Python/YAML 接入文件与测试。API 从当前仓库读取目录；无需导入历史数据库，新设备部署也能看到这一个预置。提交时冻结脚本内容和 SHA-256，不覆盖上游文件，已提交任务不随归档更新变化。维护文件后重新载入配置，修改 Hive 代码走 Hive 仓库提交。
...
平台通过上述六类环境变量传递节点和容器信息，具体编号、多节点示例见 [工作流变量](docs/workflow-variables.md)。多个作业名称使用 job0、job1、job2 等通用编号，业务含义由脚本和步骤说明表达。

Bash 入口可用 `hive_resource model 组织/权重名`、`hive_resource dataset 组织/数据集名`、`hive_resource package 包别名` 查询环境冻结的路径；不存在的映射返回非零。Python 入口由调用它的 Shell 脚本传入所需参数。上游 YAML 按提交冻结，任务所需公共脚本在预置目录维护。
...
预置表格默认显示 ModelScope 模型、量化方式、测试类型、数据集和设备，支持按列组合筛选；其余标签作为备注展示。管理员可从已有预置复制或上传工作流 JSON 添加公共预置，也可编辑已有预置的备注。普通用户可以筛选和载入配置。定义与备注保存在 MySQL；升级时执行常规 `hive.cli migrate` 创建 `009_preset_management.sql` 中的表，无需迁移旧数据。

本轮按要求未运行测试或实机任务。脚本调整需要后续实际执行验证。

环境编辑器的创建容器、安装依赖和校验环境直接展开显示。Qwen3 预置的 prepare、执行、就绪与结果校验各自使用独立脚本；job1 归档完整 AISBench 输出，job2 仅归档校验报告和输入来源，目录按任务及作业实例隔离。

公共脚本在 `preset_tasks/common` 维护，对应附件路径 `hive_presets/common`；用例通过 `common_files` 声明需要的公共附件，配置文件、IP、端口和结果路径在启动命令中传入。公共脚本会与用例附件一起冻结和编辑，不影响已提交任务。变量帮助默认收起。服务任务的后续检查对应启动后的就绪检查，普通任务对应执行完成后的检查；历史服务停止后检查保留在附加折叠区域。

脚本的来源、参数和历史文件清单见 [预置脚本来源与参数](preset_tasks/SCRIPT_ORIGINS.md)。
