...
每个任务包含多个 job，依赖通过作业图连线设置。页面按前置校验、执行、后续检查展示，各自关联具名 Shell/Python/YAML 附件，点击文件旁的“编辑”查看并修改全文；保存会同步当前任务内所有同名附件。启动命令显示如何调用文件，模型、数据集、压测参数在预置的 YAML 中修改。Qwen3 预置使用独立的创建容器、服务端安装和客户端安装脚本，以及直接读取 YAML 的服务、压测和校验入口。环境准备安装所选 vLLM-Ascend 源码及其 .github 中指定的匹配 vLLM；不解析 nightly 用例。每个步骤仅展示自身需要的附件，不再汇总所有文件；服务端不附带 AISBench 工具。不再保留未使用的历史脚本。每个 job 默认超时 10 分钟，可修改。提交时冻结实际编辑内容、SHA-256 与原文件摘要；不会修改公共归档或上游代码。上传上游文件可使用相对路径消除重名，YAML 由附件中的业务 runner 处理，Hive 不解释业务配置。
...
环境编辑器的创建容器、安装依赖和校验环境直接展开显示。Qwen3 预置的服务、压测、结果校验各自将准备和执行合并为一个入口；服务就绪使用独立 HTTP 检查；job1 归档完整 AISBench 输出，job2 仅归档校验报告和输入来源，目录按任务及作业实例隔离。
