...
function presetJobs(values: WorkflowJob[]) {
  ...
  return values.map((job, index) => {
    ...
    return { ...emptyJob(index), ...job, name: job.name || job.id, depends_on: (job.depends_on || []).map(dep => ({ ...dep, condition: dep.condition || 'succeeded' })), ports: job.ports || [], pre: presetSteps(job.pre), steps: presetSteps(job.steps), post: presetSteps(job.post), ready: presetSteps(job.ready) };
  });
}
...
export function WorkflowComposer({ onSubmitted, user, resource, onResourceChange: setResource, validateResource, initialDraft, onDraftChange, draftSaved }: { onSubmitted: () => void; user: User; resource: ResourceSpec; onResourceChange: (value: ResourceSpec) => void; validateResource: () => boolean; initialDraft?: WorkflowDraft; onDraftChange: (value: WorkflowDraft) => void; draftSaved: boolean }) {
  ...
  const [jobs, setJobs] = useState(() => initialDraft?.jobs?.length ? presetJobs(initialDraft.jobs).map(job => ({...job,environment: draftEnvironments(initialDraft).some(env => env.alias === job.environment) ? job.environment : draftEnvironments(initialDraft)[0].alias})) : [emptyJob(0,draftEnvironments(initialDraft)[0].alias)]);
