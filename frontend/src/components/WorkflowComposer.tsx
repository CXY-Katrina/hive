...
import { stepCommand, stepFiles, WorkflowFileContext } from './WorkflowFiles';
...
export function WorkflowComposer({ onSubmitted, user, resource, onResourceChange: setResource, validateResource, initialDraft, onDraftChange, draftSaved }: { onSubmitted: () => void; user: User; resource: ResourceSpec; onResourceChange: (value: ResourceSpec) => void; validateResource: () => boolean; initialDraft?: WorkflowDraft; onDraftChange: (value: WorkflowDraft) => void; draftSaved: boolean }) {
  ...
  const [retainMinutes, setRetainMinutes] = useState(initialDraft?.retainMinutes ?? 4320);
  const availableSpaces = (spaces.data || []).filter(space => (space.owner_name === user.username || user.admin) && space.status === 'READY');
  const selectedSpace = availableSpaces.find(space => String(space.id) === spaceId);
  const chooseSpace = (id: string) => {
    setSpaceId(id); setSelectedEnvironment(0); setSource(undefined); latestPr.current = ''; sourceRequest.current += 1; setResolving(false);
    const space = availableSpaces.find(value => String(value.id) === id);
    if (!space) { setEnvironments([]); return; }
    const values = space.spec.environments?.length ? space.spec.environments : Object.values(Object.fromEntries(space.environments.map(env => [env.logical_alias || env.alias, { ...env, alias: env.logical_alias || env.alias, node_aliases: space.environments.filter(value => (value.logical_alias || value.alias) === (env.logical_alias || env.alias)).map(value => value.node_alias) }])));
    const configured = values.map(env => environmentConfig({ ...env, role: env.role === 'client' ? 'client' : 'server' }));
    setEnvironments(configured); setJobs(values => values.map(job => { const alias = configured.some(env => env.alias === job.environment) ? job.environment : configured[0]?.alias || ''; return { ...job, environment: alias, node_aliases: undefined, artifacts: job.artifacts?.map(item => ({ ...item, environment: undefined, targets: undefined })) }; }));
    if (space.spec.source?.head_sha) { const { pr: _pr, branch: _branch, ...fixed } = space.spec.source; setSource({ ...fixed, revision: 'commit', commit: fixed.head_sha }); setPr(fixed.head_sha); latestPr.current = fixed.head_sha; setError(''); }
    else { setPr(''); setError('该空间缺少已核验的代码版本，暂不能提交复用任务。'); }
  };
  ...
  const newEnvironmentDraft = useRef<{ environments: WorkflowEnvironment[]; source?: WorkflowSource; pr: string } | undefined>(undefined);
  const sourceRequest = useRef(0);
  const switchReuse = (next: boolean) => {
    if (next === reuse) return;
    sourceRequest.current += 1; setResolving(false); setReuse(next); setSpaceId(''); setSelectedEnvironment(0); setError('');
    if (next) { newEnvironmentDraft.current = { environments, source, pr }; setEnvironments([]); setSource(undefined); setPr(''); latestPr.current = ''; }
    else { const saved = newEnvironmentDraft.current; const values = saved?.environments || [environment('env1', 'server'), environment('env2', 'server')]; setEnvironments(values); setSource(saved?.source); setPr(saved?.pr || ''); latestPr.current = saved?.pr || ''; setJobs(current => current.map(job => ({ ...job, environment: values.some(env => env.alias === job.environment) ? job.environment : values[0]?.alias || '', node_aliases: undefined, artifacts: job.artifacts?.map(item => ({ ...item, environment: undefined, targets: undefined })) }))); }
  };
  ...
  useEffect(() => { onDraftChange({ name, pr, source, environments, jobs, reuse, spaceId, retainMinutes, presetId, presetName, presetTags, presetYamlPath, selectedEnvironment, selectedJobId }); }, [name, pr, source, environments, jobs, reuse, spaceId, retainMinutes, presetId, presetName, presetTags, presetYamlPath, selectedEnvironment, selectedJobId, onDraftChange]);
  ...
  const submit = async (event: FormEvent) => {
    ...
    const paths = jobs.flatMap(job => [...job.pre, ...job.steps, ...job.post, ...job.ready]).concat(reuse ? [] : environments.flatMap(env => [env.bootstrap, ...env.install, ...env.verify]));
    ...
    if (deriving) {
      ...
      const workflow = reuse ? { ...currentWorkflow, space_id: undefined, resource: { ...defaultResource(), ...selectedSpace!.spec.resource, min_memory_gib: 0 }, environments: environments.map(env => ({ ...env, node_alias: undefined, node_aliases: env.node_aliases || [env.node_alias], bootstrap: serialize(env.bootstrap), install: env.install.map(serialize), verify: env.verify.map(serialize) })) } : currentWorkflow;
      setBusy(true);
  ...
  return <WorkflowFileContext.Provider value={{files: sharedFiles, save: saveSharedFile}}><section className="panel workflow-composer section-gap"><div className="panel-title"><span className="title-icon"><Icon name="task" /></span><div><h2>新建容器任务</h2><p>固定代码版本、独立环境与作业依赖</p></div></div><form ref={formRef} noValidate onSubmit={event => void submit(event)}><div className="panel-body">{!canRequest && <p className="notice">尚未获得服务器申请权限，请联系管理员开启任务提交。</p>}<WorkflowPresets admin={user.admin} source={source} onApply={preset => { if (preset.loadable === false || !preset.workflow) return; setError(''); try { const snapshot = structuredClone(preset.workflow); let nextJobs = presetJobs(snapshot.jobs || []); if (reuse) nextJobs = nextJobs.map(job => ({ ...job, environment: environments.some(env => env.alias === job.environment) ? job.environment : environments[0]?.alias || '', node_aliases: undefined, artifacts: job.artifacts?.map(item => ({ ...item, environment: undefined, targets: undefined })) })); let nextEnvironments: WorkflowEnvironment[] | undefined; if (snapshot.environments && !reuse) { if (!Array.isArray(snapshot.environments)) throw new Error('预置环境列表格式无效。'); nextEnvironments = snapshot.environments.map(env => { if (!env || typeof env.alias !== 'string' || env.role && !['server', 'client'].includes(env.role)) throw new Error('预置环境代称或角色无效。'); return { ...environment(env.alias, env.role || 'server'), ...env, install: presetSteps(env.install), verify: presetSteps(env.verify), packages: env.packages || [], environment: env.environment || {} }; }); } if (!reuse) { const baseline = nextEnvironments || environments; nextJobs = nextJobs.map(job => ({...job, artifacts: job.artifacts?.map(item => { const expected = environmentTargets(baseline, job.environment); const matchesDefault = item.targets?.length === expected.length && expected.every(target => item.targets?.some(value => value.environment === target.environment && value.node_alias === target.node_alias)); return matchesDefault ? {...item,targets:undefined,environment:undefined} : item; })})); } setPresetYamlPath(preset.origin === 'archive' && preset.scope !== 'personal' ? preset.yaml_path : undefined); setPresetId(preset.id); setPresetName(preset.name); setPresetTags(Object.fromEntries(Object.entries(preset.tags || {}).map(([key,value]) => [key, Array.isArray(value) ? value.join(' / ') : value]))); if (!reuse) { setSource(undefined); setPr('main'); latestPr.current = 'main'; void resolveReference('main',preset); } setSelectedEnvironment(0); setSelectedJobId(nextJobs[0]?.id); setName(snapshot.name || preset.name); setJobs(nextJobs); if (nextEnvironments) setEnvironments(nextEnvironments); if (snapshot.resource && !reuse) setResource({ ...defaultResource(), ...snapshot.resource, min_memory_gib: 0 }); setNotice(`已载入「${preset.name}」的外部执行配置，请确认代码版本与环境输入后提交。`); } catch (err) { setError(errorText(err)); } }} />
    <div className="form-grid"><Field label="任务名称"><input required maxLength={128} value={name} onChange={e => setName(e.target.value)} /></Field><Field label="vLLM-Ascend PR / commit" hint="默认 main；也可填写 PR 编号、PR 链接或完整的 40 位 commit SHA。"><div className="workflow-inline"><input aria-label="vLLM-Ascend PR / commit" required disabled={reuse} value={pr} onChange={e => { setPr(e.target.value); latestPr.current = e.target.value; setSource(undefined); }} /><button type="button" className="button secondary small-button" disabled={reuse || resolving || !pr.trim()} onClick={() => void resolveSource()}>{resolving ? '解析中…' : '解析 PR / commit'}</button></div></Field></div>
