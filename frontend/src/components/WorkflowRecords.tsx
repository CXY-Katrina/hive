...
export function WorkflowRecords({ refresh, user }: { refresh: number; user: User }) {
  ...
  return <><section className="panel section-gap" aria-label="任务记录"><div className="panel-toolbar"><h2>任务记录 <span className="count-chip">{query.data?.length ?? 0}</span></h2><button className="text-button" onClick={() => void query.reload()}>刷新任务记录</button></div><p className="muted small" style={{ padding: '0 20px' }}>{user.admin ? '管理员可查看全部任务' : '当前显示本人任务'}</p><ErrorNotice text={query.error || error} />{notice && <div className="notice success" role="status">{notice}</div>}{!query.data && query.loading ? <Loading /> : !query.data && query.error ? <Empty title="任务记录暂时无法读取" detail="请刷新重试；当前状态无法确认任务数量。" /> : !query.data?.length ? <Empty title={user.admin ? '当前暂无任务记录' : '当前账号暂无任务'} detail="排队任务提交后即在此显示。" /> : <div className="record-list">{query.data.map(run => {
    ...
    {selected && <WorkflowDetail initial={selected} actualEnvironments={spaces.data?.find(space => String(space.id) === String(selected.space_id))?.environments || []} onClose={() => setSelected(undefined)} />}
...
function WorkflowAllocation({run,actual}:{run:WorkflowRun;actual:WorkflowSpace['environments']}) {
  const planned=(run.spec?.environments||[]).flatMap(env=>[...(env.node_aliases||[env.node_alias])].filter(Boolean).sort((a,b)=>Number(a.slice(4))-Number(b.slice(4))).map(node=>({alias:env.alias,node})));
  const rows=(planned.length?planned:actual.map(env=>({alias:env.logical_alias||env.alias,node:env.node_alias}))).map((row,index)=>({...row,index,actual:actual.find(env=>(env.logical_alias||env.alias)===row.alias&&env.node_alias===row.node)}));
  return <section className="workflow-allocation"><h4>实际服务器与容器</h4>{!actual.some(env=>env.host||env.container_name)&&<p className="muted small">尚未返回分配结果；下面的环境与节点为申请代称。</p>}<div className="table-scroll"><table><thead><tr><th>环境 / 节点</th><th>服务器 IP</th><th>真实容器名</th><th>变量对应</th></tr></thead><tbody>{rows.map(row=><tr key={`${row.alias}/${row.node}`}><td>{row.alias}<small className="subline">{row.node}</small></td><td><code>{row.actual?.host||'待分配'}</code></td><td><code>{row.actual?.container_name||'待创建'}</code>{row.actual?.status&&<small className="subline"><Badge value={row.actual.status}/></small>}</td><td><code>HIVE_NODE{Number(row.node.slice(4))}_IP</code><small className="subline"><code>HIVE_CONTAINER{row.index}_NAME</code></small></td></tr>)}</tbody></table></div></section>;
}

function WorkflowDetail({ initial, actualEnvironments, onClose }: { initial: WorkflowRun; actualEnvironments: WorkflowSpace['environments']; onClose: () => void }) {
  const query = useQuery<WorkflowRun>(`/workflows/${initial.id}`, 5000);
  const run = query.data || initial;
  return <Modal title={`${run.name} · 任务详情`} onClose={onClose} wide><div className="modal-body"><ErrorNotice text={query.error} /><div className="workflow-row-heading"><Badge {...runStatus(run)} /><small className="muted">每 5 秒刷新 · #{shortId(run.id)}</small></div><PendingStatus status={run.status} />{run.reason && <p className="record-reason">{run.reason}</p>}<dl className="detail-list source-snapshot"><dt>执行代码 SHA</dt><dd><code>{run.spec?.source?.head_sha || '待固定'}</code></dd><dt>匹配 vLLM SHA</dt><dd><code>{run.spec?.source?.vllm_sha || '待固定'}</code></dd><dt>开始 / 结束</dt><dd>{dateTime(run.started_at)} / {dateTime(run.finished_at)}</dd></dl><WorkflowAllocation run={run} actual={actualEnvironments}/>{run.spec?.jobs?.length > 0 && <WorkflowGraph jobs={run.spec.jobs} readOnly />}
    {run.jobs?.length ? run.jobs.map(job => <section className="workflow-job-run" key={job.id}><div className="record-heading"><h3>{job.name || job.id}</h3><Badge value={job.status} label={job.status === 'SUCCEEDED' ? '执行成功' : undefined} /></div><p className="muted small">作业 {job.logical_job_id || job.id}{job.node_alias && ` · ${job.node_alias}`} · 阶段 {workflowPhaseLabel(job.phase)}{job.exit_code != null && ` · 退出码 ${job.exit_code}`}</p><p className="muted small">产物目录：<code>{`/var/tmp/hive/outputs/${run.id}/${job.id}`}</code></p><PendingStatus status={job.status} job />{job.reason && <p className="record-reason">{job.reason}</p>}{job.endpoint && <p>服务地址：<code>{typeof job.endpoint === 'string' ? job.endpoint : JSON.stringify(job.endpoint)}</code></p>}<WorkflowEvidence workflowId={run.id} job={job} /></section>) : <Empty title="作业等待调度" detail="环境准备完成后，在这里查看各阶段状态及独立日志。" />}
  </div></Modal>;
}
