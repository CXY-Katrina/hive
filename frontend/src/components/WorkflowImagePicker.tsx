...
export function WorkflowImagePicker({label,value,onChange}:{label:string;value:string;onChange:(value:string)=>void}) {
  ...
  return <div className="workflow-image-picker"><Field label={label}><input required value={value} onChange={event=>onChange(event.target.value)} placeholder="现有镜像、映射别名或 Quay 镜像" /></Field><details onToggle={event=>{if(event.currentTarget.open && !page)void load();}}><summary>搜索 Quay 镜像标签</summary><input aria-label="搜索镜像标签" value={search} onChange={event=>setSearch(event.target.value)} placeholder="搜索已加载的标签" /><div className="image-tag-options">{matches.map(tag=><button type="button" className="text-button" key={tag.name} title={tag.image} onClick={()=>onChange(tag.image)}>{tag.name}</button>)}</div>{!busy&&!matches.length&&<p className="muted small">暂无匹配标签，可继续加载或手工填写镜像。</p>}<ErrorNotice text={error} />{hasMore&&<button type="button" className="text-button" disabled={busy} onClick={()=>void load()}>{busy?'正在加载标签…':error?'重试加载标签':'加载更多标签'}</button>}<p className="muted small">镜像选择只修改配置；需要时在环境准备阶段拉取。可随时手工填写本机镜像或映射别名。</p></details></div>;
}
