...
export function NodesPage({ nodeId }: { nodeId?: string }) {
  ...
  const [detail, setDetail] = useState<{ node: NodeInfo; device: Device }>();
  ...
  const filtered = useMemo(() => (query.data || []).filter(node => {
    ...
    const contents = [node.name, node.host, node.model, node.cluster_name, ...node.devices.flatMap(device => [device.owner_name || '', ...device.processes.flatMap(process => [String(process.pid), process.container_name || ''])])].join(' ').toLowerCase();
  ...
  {detail && <DeviceDetail node={detail.node} device={detail.device} descriptors={metrics.data || []} onClose={() => setDetail(undefined)} />}</>;
}

function probeLabel(metadata: Record<string, unknown>) {
  const admission = metadata?.admission as { checks?: number; passed?: number; peer_count?: number } | undefined;
  if (admission && admission.checks != null) return `${admission.passed || 0} / ${admission.checks} 项通过 · ${admission.peer_count || 0} 个对端`;
