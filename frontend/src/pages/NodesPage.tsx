...
import { NodeMappings } from '../components/NodeMappings';
...
export function NodesPage({ nodeId }: { nodeId?: string }) {
  ...
  const extraMetrics = (metrics.data || []).filter(metric => !['ai_core', 'memory_used', 'memory_total'].includes(metric.key) && (query.data || []).some(node => node.devices.some(device => device.extensions && metric.key in device.extensions)));
  ...
  <div className="toolbar"><div className="search-input"><Icon name="search" size={18} /><input aria-label="搜索节点、申请人或容器" placeholder="搜索 IP、机型、申请人、Docker 容器…" value={search} onChange={e => setSearch(e.target.value)} /></div><select aria-label="筛选代际" value={generation} onChange={e => setGeneration(e.target.value)}><option value="">全部代际</option>{['A2', 'A3', 'A5'].map(g => <option key={g}>{g}</option>)}</select><select aria-label="筛选卡状态" value={status} onChange={e => setStatus(e.target.value)}><option value="">全部状态</option>{['available', 'allocated', 'external', 'unknown', 'maintenance', 'fault'].map(s => <option key={s} value={s}>{statusLabel(s)}</option>)}</select>{extraMetrics.length > 0 && <label className="check-field simple" title={extraMetrics.map(metric => metric.label || metric.key).join("、")}><input type="checkbox" checked={extensions} onChange={e => setExtensions(e.target.checked)} />更多采集指标</label>}</div>
  {extensions && <ErrorNotice text={metrics.error} retry={() => void metrics.reload()} />}
  {!query.data && query.loading ? <Loading /> : !query.data && query.error ? <div className="panel"><Empty title="暂时无法读取节点信息" detail="请检查连接后重试。" /></div> : filtered.length === 0 ? <div className="panel"><Empty title={query.data?.length ? '没有匹配的节点' : '还没有纳管节点'} detail={query.data?.length ? '尝试调整搜索或筛选条件。' : '纳管机器后，这里会显示所有节点的实时 NPU 状态。'} action={!query.data?.length && <Link className="button primary" to="/clusters">前往集群管理<Icon name="arrow" size={16} /></Link>} /></div> : <div className="node-list">{filtered.slice((currentPage - 1) * pageSize, currentPage * pageSize).map(node => <article className="node-panel panel" key={node.id}><div className="node-heading"><div className="node-identity"><span className="node-icon"><Icon name="server" /></span><div><h2><Link to={`/nodes/${node.id}`}>{node.name || node.host}</Link><span className="generation">{node.generation}</span></h2><span className="muted mono">{node.host}</span><span className="node-model">{modelLabel(node)}{clusterLabel(node) && ` · ${clusterLabel(node)}`}</span><HardwareSummary node={node} /></div></div><div className="node-heading-right"><Badge value={node.maintenance ? 'maintenance' : node.status} /><small>采样 {dateTime(node.sampled_at)}</small></div></div>{node.reason && <div className="node-reason">{node.reason}</div>}<div className="table-scroll"><table className="device-table"><thead><tr><th>NPU 卡</th><th>AI Core</th><th>显存 HBM</th><th>平台登记</th><th>实际进程 / Docker 容器</th><th>状态</th></tr></thead><tbody>{node.devices.map(device => <DeviceRow key={device.id} device={device} metrics={extensions ? extraMetrics : []} receivedAt={query.updatedAt?.getTime()} onDetail={() => setDetailSelection({ nodeId: node.id, deviceId: device.id })} />)}</tbody></table>{!node.devices.length && <p className="inline-empty">尚未发现 NPU 设备，等待节点检测与采样。</p>}</div><div className="node-footer"><span><Icon name="server" size={14} />互联：{probeLabel(node.metadata)}</span><span>共享盘：{node.mounts?.length ? node.mounts.map(mount => `${mount.target || mount.path || '未知路径'} · ${statusLabel(mount.status)}`).join('，') : '未发现 / 待检测'}</span></div>{nodeId && <NodeMappings nodeId={node.id} />}</article>)}</div>}
