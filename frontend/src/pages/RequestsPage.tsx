...
export function RequestsPage({ user }: { user: User }) {
  ...
  return <><PageHeader eyebrow="RESOURCE REQUESTS" title="机器申请" description="按需选择机器与卡，分配结果包含 IP、卡号和连接凭据。"><span className="owner-pill"><Icon name="user" size={16} />申请人：{user.username}</span></PageHeader><label className="check-field workflow-mode"><input type="checkbox" checked={createTask} onChange={event => setCreateTask(event.target.checked)} /><span>新建任务</span></label>{createTask ? <WorkflowComposer user={user} onSubmitted={() => { setWorkflowRefresh(value => value + 1); void query.reload(); }} /> : <div className="split-layout"><section className="panel request-form-panel"><div className="panel-title"><span className="title-icon"><Icon name="request" /></span><div><h2>申请调试资源</h2><p>资源按完整申请集合交付</p></div></div><form onSubmit={event => void submit(event)}><div className="panel-body">{!canRequest && <p className="notice">尚未获得服务器申请权限，请联系管理员开启。现有申请仍可归还。</p>}<ResourceForm value={spec} onChange={setSpec} /><ErrorNotice text={error} />{notice && <div className="notice success" role="status"><Icon name="check" size={17} /><span>{notice}</span></div>}</div><div className="form-footer"><span>{resourceDescription(spec)}</span><button className="button primary" disabled={busy || !canRequest}>{busy ? '正在处理…' : '提交申请'}<Icon name="arrow" size={16} /></button></div></form></section><aside className="policy-panel"><h3>调试资源使用约定</h3><div className="policy-time"><strong>30</strong><span>分钟<br />交付保护期</span></div><ol><li><strong>按卡分配</strong><p>拿到资源后，只使用分配给你的卡。</p></li><li><strong>按实际使用保留</strong><p>保护期后，只要申请卡集合仍有 AI Core 活动，继续保留。</p></li><li><strong>空闲后自动归还</strong><p>全部申请卡连续 10 分钟 AI Core 为零，可清理关联进程并释放；缺测不视为空闲。</p></li></ol></aside></div>}
