...
import { useRequestDraft, type WorkflowDraft } from '../workflowDrafts';
import { WorkflowComposer } from '../components/WorkflowComposer';
import { WorkflowRecords } from '../components/WorkflowRecords';
import { Assignments } from '../components/Assignments';
import { ResourceForm, resourceDescription } from '../components/ResourceForm';
...
export function RequestsPage({ user }: { user: User }) {
  const canRequest = user.admin || user.can_request;
  const query = useQuery<ResourceRequest[]>('/requests', 15000);
  const draft = useRequestDraft(user.username);
  const spec = draft.value.resource, createTask = draft.value.createTask;
  const setSpec = useCallback((resource: typeof spec) => draft.update(value => ({ ...value, resource })), [draft.update]);
  const setCreateTask = useCallback((change: boolean | ((previous: boolean) => boolean)) => draft.update(value => ({ ...value, createTask: typeof change === 'boolean' ? change : change(value.createTask) })), [draft.update]);
  const saveWorkflow = useCallback((workflow: WorkflowDraft) => draft.update(value => ({ ...value, workflow })), [draft.update]);
  ...
  const submit = async (event: FormEvent) => {
    event.preventDefault(); if (busy || !canRequest || createTask) return; setBusy(true); setError(''); setNotice('');
    try {
  ...
  if (!draft.ready) return <Loading />;
  return <><PageHeader eyebrow="RESOURCE REQUESTS" title="机器申请" description="按需选择机器与卡，分配结果包含 IP、卡号和连接凭据。"><span className="owner-pill"><Icon name="user" size={16} />申请人：{user.username}</span></PageHeader><div className={createTask ? 'workflow-resource-first' : 'split-layout'}><section className="panel request-form-panel"><div className="panel-title"><span className="title-icon"><Icon name="request" /></span><div><h2>资源申请</h2><p>资源按完整申请集合交付</p></div><button type="button" className="button primary workflow-mode" aria-pressed={createTask} onClick={() => setCreateTask(value => !value)}>{createTask ? '取消新建任务' : '+ 新建任务'}</button></div><form ref={resourceForm} onSubmit={event => void submit(event)}><div className="panel-body">{!canRequest && <p className="notice">尚未获得服务器申请权限，请联系管理员开启。现有申请仍可归还。</p>}<ErrorNotice text={draft.error} /><ResourceForm value={spec} onChange={setSpec} task={createTask} /><ErrorNotice text={error} />{notice && <div className="notice success" role="status"><Icon name="check" size={17} /><span>{notice}</span></div>}</div>{!createTask && <div className="form-footer"><span>{resourceDescription(spec)}</span><button className="button primary" disabled={busy || !canRequest}>{busy ? '正在处理…' : '提交申请'}<Icon name="arrow" size={16} /></button></div>}</form></section>{!createTask && <aside className="policy-panel"><h3>调试资源使用约定</h3><div className="policy-time"><strong>30</strong><span>分钟<br />交付保护期</span></div><ol><li><strong>按卡分配</strong><p>拿到资源后，只使用分配给你的卡。</p></li><li><strong>按实际使用保留</strong><p>保护期后，只要申请卡集合仍有 AI Core 活动，继续保留。</p></li><li><strong>空闲后自动归还</strong><p>全部申请卡连续 10 分钟 AI Core 为零，可清理关联进程并释放；缺测不视为空闲。</p></li></ol></aside>}</div>{createTask && <WorkflowComposer initialDraft={draft.value.workflow} onDraftChange={saveWorkflow} draftSaved={draft.saved} resource={spec} onResourceChange={setSpec} validateResource={() => resourceForm.current?.reportValidity() ?? true} user={user} onSubmitted={() => { setWorkflowRefresh(value => value + 1); void query.reload(); }} />}
  <WorkflowRecords refresh={workflowRefresh} user={user} />
  <section className="panel section-gap"><div className="panel-toolbar"><h2>申请记录 <span className="count-chip">{rows.length}</span></h2><div className="toolbar-actions"><label className="check-field simple"><input type="checkbox" checked={mine} onChange={e => setMine(e.target.checked)} />只看我的</label><button className="icon-button" title="刷新申请记录" aria-label="刷新申请记录" onClick={() => void query.reload()} disabled={query.loading}><Icon name="refresh" size={17} /></button></div></div><ErrorNotice text={query.error} retry={() => void query.reload()} />{!query.data && query.loading ? <Loading /> : !query.data && query.error ? <Empty title="暂时无法读取申请记录" detail="请检查连接后重试。" /> : !rows.length ? <Empty title="暂无申请记录" detail="提交申请后，在这里查看分配进度和连接信息。" /> : <div className="record-list">{rows.map(item => <article className="request-record" key={item.id}><div className="record-heading"><div><strong>#{shortId(item.id)}</strong><span className="muted">{item.owner_name} · {dateTime(item.created_at)}</span></div><Badge value={item.status} /></div><p className="record-spec">{resourceDescription(item.spec)}{item.spec?.require_interconnect && <span className="tag">多机互联</span>}{item.purpose === 'task' && <span className="tag">任务资源</span>}</p>{item.spec?.note && <p className="record-reason">用途：{item.spec.note}</p>}{item.reason && <p className="record-reason">{item.reason}</p>}<Assignments devices={item.devices || []} showCredentials={user.admin || user.can_view_credentials} />{item.protected_until && <p className="protected-line"><Icon name="clock" size={14} />保护至 {dateTime(item.protected_until)}</p>}{!finalStates.includes(item.status.toLowerCase()) && item.purpose !== 'task' && (user.admin || item.owner_name === user.username) && <div className="record-actions"><button className="button secondary small-button" onClick={() => { setRelease(item); setError(''); }}>{['queued', 'pending'].includes(item.status.toLowerCase()) ? '取消申请' : '归还资源'}</button></div>}</article>)}</div>}</section>
