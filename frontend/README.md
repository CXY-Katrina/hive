...
当前展示页面：`/login`、`/clusters`、`/nodes/`（及 `/nodes/:id`）、`/requests`。任务提交和记录集成在“机器申请”：点击“+ 新建任务”配置固定 PR、独立 server/client 环境、多个文件入口与作业依赖。独立任务中心菜单仍隐藏，`/tasks` 及子路径重定向到 `/nodes/`；旧 TasksPage 源码保留兼容。页面直接读取 API，无演示节点或模拟运行数据。
...
- `WorkflowEnvironmentEditor` / `WorkflowFiles`：节点/容器与镜像配置、Shell/Python/YAML 多文件上传和启动命令。Shell/Python 留空可依次执行，YAML 需要命令读取；服务器按文件名匹配固定 PR 并核对执行代码，重名时可在文件旁选择重命名，补充 PR 相对路径。
- `WorkflowGraph` / `WorkflowJobEditor`：环境与 Jobs 分页，React/SVG 连线编辑依赖；服务 READY 条件、前后检查和零 NPU 客户端。作业默认 10 分钟，产物按目标环境、名称和路径声明，不要求选择文件类型。
