...
class NightlyCLITest(unittest.TestCase):
    ...
    def test_archived_cli_can_be_imported_without_upstream_tools_or_sys_path_changes(self):
        ...
        case, benchmark = module.select_case(CONFIG, CASE, "perf")
        self.assertEqual(case["name"], CASE)
        self.assertEqual(benchmark["baseline"], 812.3394)
...
""",
    ...
    def test_locate_uses_current_log_and_copies_exact_results_for_explicit_verification(self):
        ...
        self.assertIn(str(ROOT / "nightly_cli.py"), (archive / "verify.sh").read_text())
        self.assertNotIn("tools.nightly_cli", (archive / "verify.sh").read_text())
