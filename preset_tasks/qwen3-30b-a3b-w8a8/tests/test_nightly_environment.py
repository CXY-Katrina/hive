...
SOURCE = Path(__file__).parent / "fixtures/upstream"
SOURCE_COMMIT = "d4d2957e5208c2f464d4625c05920bd29ea233cb"
_spec = importlib.util.spec_from_file_location("nightly_environment", ARCHIVE / "nightly_environment.py")
nightly_environment = importlib.util.module_from_spec(_spec)
sys.modules["nightly_environment"] = nightly_environment
_spec.loader.exec_module(nightly_environment)
cann_environment = nightly_environment.cann_environment
check_baseline = nightly_environment.check_baseline
main = nightly_environment.main
new_conflicts = nightly_environment.new_conflicts
validate_pip_check = nightly_environment.validate_pip_check
validate_sha = nightly_environment.validate_sha
web_requirements = nightly_environment.web_requirements


class ImageReuseTests(unittest.TestCase):
    def test_wrong_upstream_commit_is_rejected_before_environment_creation(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            with (
                ...
                patch.object(subprocess, "check_output", return_value="f" * 40),
                contextlib.redirect_stderr(error),
                self.assertRaises(SystemExit) as failure,
            ):
                main()
            self.assertEqual(failure.exception.code, 2)
            self.assertIn("upstream checkout HEAD", error.getvalue())
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            def check_output(command, **kwargs):
                ...
                if command[-2:] == ["rev-parse", "HEAD"]:
                    return (
                        nightly_environment.AISBENCH_SHA if "benchmark" in str(command[2]) else SOURCE_COMMIT
                    ) + "\n"
...
class BaselineImageTests(unittest.TestCase):
    ...
    def test_vendor_script_failure_stops_loading_in_real_shell(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            with (
                patch("nightly_environment.CANN_SCRIPTS", (cann.as_posix(),)),
                patch("nightly_environment.ATB_SCRIPT", atb.as_posix()),
            ):
